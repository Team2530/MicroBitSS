...
document.addEventListener("keyup", (event) => {
  if (event.key === "b") {toggleScoreboard();}
});

function toggleScoreboard() {
  console.log(timePassed, initialTime, timerInterval,timePassed === initialTime,!matchesJSON || timerInterval == null || 
    !(timePassed === 0 || timePassed === initialTime))
  if (!matchesJSON) {alert("No matches loaded"); return;}
  if (!timerInterval == null || !(timePassed === 0 || timePassed === initialTime)) {return;}
...
function createScoreboard() {
  if (!matchesJSON || !matchTable || !teamTable) {return;}
...
function createBracket() {
  if (matchesJSON == null) {return;}
  if (!confirm("Generating bracket will clear all other match data, do you wish to continue?")) {return;}
  const sortedTeams = Object.entries(teamMap).sort((a, b) => b[1].wins - a[1].wins || b[1].score - a[1].score);
  if (sortedTeams.length < 4) {return;}
